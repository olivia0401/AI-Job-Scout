...
> 打开一个 **AI 助手**（ChatGPT、Claude、Gemini 等都行）粘给它、问「怎么办」，它会手把手教你。这是你的万能求助按钮。
...
| 其它任何看不懂的报错 | **整段复制**，粘给任意 **AI 助手**（ChatGPT / Claude / Gemini）问「怎么解决」。真的，这招最省事。 |
...
### ④ 看结果 / 挑岗位 / 要建议
- 每个岗位行左侧就是**匹配度色块**（绿=高、黄=中），算过分后默认按匹配度排序。
- **点任意岗位行展开**：看你缺哪些技能 → 一键生成该岗位的 **✍️ 简历修改建议** 和
  **💬 networking 搭话草稿**（LinkedIn 申请附言 / 跟进私信 / 冷邮件，个性化生成，复制即用）。
- 想投的点 **☆ 收藏** → 顶部 **📌 投递清单** 里统一管理：改状态（待投→已投→面试中→Offer/已拒）、
  一键打开申请页、找招聘官、查邮箱。投递动作本身保留人工——每一份申请都用定制过的简历，
  比任何"自动海投"都有效（海投工具还容易被平台封号）。
- 点 **📥 导出** 存成 Excel；点简历卡上的 **🎯 简历提升建议** 看「全平台最该补的技能」。
...
> 不确定填什么词？问任意 **AI 助手**：「我想找 XX 行业的英国工签岗位，帮我列一组关键词」。

---

# 🤖 想改点什么？让 AI 助手帮你（零基础也能改）

不用学编程。用任意 **AI 编程助手**，或直接把项目文件夹发给 ChatGPT / Claude 等 **AI 助手**，
用**大白话**说你要什么就行，比如：
...
你只描述**想要的结果**，AI 助手负责改代码并解释。小贴士：让它改完顺手跑一下 `pytest tests/` 确认没改坏。
...
没配大模型 key 时，旁边的 **📋 复制 Prompt** 仍可把整段提示词复制出去，贴到 ChatGPT / Claude 等大模型手动生成。
